  ...
  if (NET_EVENT_STATE_CHANGE == event_class)
  {
    ...
    switch(new_state)
    {
      case NET_STATE_INITIALIZED:
        PMAWS_PRINTF("- Network Interface initialized: \n");
        break;

      case NET_STATE_STARTING:
        PMAWS_PRINTF("- Network Interface starting: \n");
        break;

      case NET_STATE_READY:
        PMAWS_PRINTF("- Network Interface started: \n");
        PMAWS_PRINTF("   - Device Name : %s. \n", netif->DeviceName );
        PMAWS_PRINTF("   - Device ID : %s. \n", netif->DeviceID );
        PMAWS_PRINTF("   - Device Version : %s. \n", netif->DeviceVer );
        break;

      case NET_STATE_CONNECTING:
        PMAWS_PRINTF("- Network Interface connecting: \n");
        break;

      case NET_STATE_CONNECTED:
        PMAWS_PRINTF("- Network Interface connected: \n");
        PMAWS_PRINTF("   - IP address :  %s. \n", net_ntoa(&netif->ipaddr));
        break;

      case NET_STATE_DISCONNECTING:
        PMAWS_PRINTF("- Network Interface disconnecting\n");
        break;

      case NET_STATE_STOPPING:
        PMAWS_PRINTF("- Network Interface stopping\n");
        break;

      case NET_STATE_DEINITIALIZED:
        PMAWS_PRINTF("- Network Interface de-initialized\n");
        break;
...
void CLOUD_Error_Handler(int errorCode)
{
  switch (errorCode)
  {
    ...
    {
      PMAWS_PRINTF("Error initializing the module!\n");

      break;
    }
    case (CLOUD_DEMO_MAC_ADDRESS_ERROR):
    {
      PMAWS_PRINTF("Error detecting module!\n");

      break;
    }
    case (CLOUD_DEMO_WIFI_CONNECTION_ERROR):
    {
      PMAWS_PRINTF("Error connecting to AP!\n");

      break;
    }
    case (CLOUD_DEMO_IP_ADDRESS_ERROR):
    {
      PMAWS_PRINTF("Error retrieving IP address!\n");

      break;
    }
    case (CLOUD_DEMO_CONNECTION_ERROR):
    {
      PMAWS_PRINTF("Error connecting to Cloud!\n");

      break;
    }
    case (CLOUD_DEMO_TIMEDATE_ERROR):
    {
      PMAWS_PRINTF("Error initializing the RTC from the network time!\n");
...
int platform_init(void)
{
  ...
  PMAWS_PRINTF("*************************************************************\n");
  PMAWS_PRINTF("   %s                                                  \n", FW_VERSION_NAME);
  PMAWS_PRINTF("   Predictive Maintenance Cloud application            \n");
  PMAWS_PRINTF("   for STWIN Kit                                       \n");
  PMAWS_PRINTF("   FW version %d.%d.%d - %s                            \n",
  ...
  {
    PMAWS_PRINTF("Push the User button (USR) within the next 5 seconds if you want to update "
...
void platform_deinit()
{
  ...
  PMAWS_PRINTF("Cloud connectivity demonstration completed\n");
